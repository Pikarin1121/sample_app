  ...
  describe '投稿画面(todolists_new_path)のテスト' do
    ...
    context '表示の確認' do
      it 'todolists_new_pathが"/todolists/new"であるか' do
        expect(currnt_path).to eq '/todolists/new'
      end
    ...
    context '投稿処理のテスト' do
      it '投稿後のリダイレクト先は正しいか' do
        ...
        click_button '投稿'
        expect(page).to have_current_path todolists_new_path(List.last)
      end
    end
  end
  ...
  describe '詳細画面のテスト' do
    ...
    context 'リンク先の確認' do
      it '編集の遷移先は編集画面か' do
        ...
        edit_link.click
        expect(currnt_path).to eq('/todolists/' + list.id.to_s + '/edit')
      end
    end
  ...
  describe '編集画面のテスト' do
    ...
    context '更新処理に関するテスト' do
      it '更新後のリダイレクト先は正しいか' do
        ...
        fill_in 'list[body]', with: Faker::Lorem.characters(number:20)
        click_button '保存'
        expct(page).to have_current_path todolist_path(list)
      end
    end
  end
end